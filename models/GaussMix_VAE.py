...
from variational_coders.encoders import GaussMixEncoder
from variational_coders.decoders import Decoder

### Mixture of Gaussians VAE ###
class GaussianMix_VAE(object):
    def __init__(self, rng, input, batch_size, layer_sizes, 
        ...
        # init encoder
        self.encoder = GaussMixEncoder(rng, input=next_layer_input, batch_size=batch_size, in_size=layer_sizes[-1], latent_size=latent_size)
        ...
        self.post_mu_arr = self.encoder.mu_arr
        self.post_log_sigma_arr = self.encoder.log_sigma_arr
